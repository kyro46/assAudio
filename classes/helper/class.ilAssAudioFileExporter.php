...
/**
 * @author		Original: Björn Heyser <bheyser@databay.de>
 * 
 *              
 *              Adaption for assAudio from Modules/TestQuestionPool/classes/class.ilAssFileUploadUploadsExporter.php
 * 
 * @version		$Id$
 *
 * @package     Modules/Test
 */
class ilAssAudioFileExporter
{
        ...
        foreach ($solution_data as $activeId => $passes) {
            ...
            foreach ($passes as $pass => $files) {
                foreach ($files as $file) {
                    ...
                    // Filename
                    $value1 = $file['value1'] ?? null;
                    $value2 = $file['value2'] ?? null;
                    
                    if (is_string($value1) && $value1 !== '') {
                        $filename = $value1;
                    } else {
                        $filename = 'file_' . md5(json_encode($file));
                    }
                    ...
                    $legacy_file_path = $file_dir . $value1;
                    $resource = fopen($legacy_file_path, 'rb');
                    $stream = Streams::ofResource($resource);
                    
                    if ($stream instanceof \ILIAS\Filesystem\Stream\Stream) {
                        $streams[$dir . $filename] = $stream;
                    }
                }
            }
        }
